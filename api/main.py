...
PLACEHOLDER_URL = "https://source.unsplash.com/random/300x300/?dark,abstract"

FALLBACK_THEME = "spotify-light.html.j2"
...
def makeSVG(request, data, background_color, border_color):
    barCount = 84
    contentBar = "".join(["<div class='bar'></div>" for _ in range(barCount)])
    barCSS = barGen(barCount)
    
    template_name = getTemplate()
...
@app.get("/")
def read_root():
    current_song = getRedisValue("current-song")
    current_song = json.loads(current_song) if current_song else {}
    
    if "is_playing" in current_song:
        data = {
            "progress_ms": current_song.get("progress_ms", 0),
            "is_playing": current_song.get("is_playing", False),
            "item": current_song.get("item", {}),
            "context": current_song.get("context", {})
        }
    else:
        # Access the first item in the 'items' list if it exists.
        first_item = current_song.get("items", [])[0] if current_song.get("items") else None
        
        if first_item:
            data = {
                "played_at": first_item.get("played_at", "No song currently playing"),
                "item": first_item.get("track", {})
            }
        else:
            # Handle the case where there are no items
            data = {
                "played_at": "No song currently playing",
                "item": {}
            }
...
@app.get("/top-tracks")
def read_top_tracks():
    top_tracks = getRedisValue("top-tracks")
